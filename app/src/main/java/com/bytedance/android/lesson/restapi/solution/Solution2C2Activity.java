...
public class Solution2C2Activity extends AppCompatActivity {
    ...
    private void initRecyclerView() {
        ...
        mRv.setAdapter(new RecyclerView.Adapter() {
            ...
            public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
                ...
                Url url = mFeeds.get(i).getImage_url();
    ...
    public void fetchFeed(View view) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                String url = "http://10.108.10.39:8080/";
                Retrofit retrofitted = RetrofitManager.get(url);
                Call<List<Feed>> creed = retrofitted.create(IMiniDouyinService.class).getFeed();
                try {
                    Response<List<Feed>> reed = creed.execute();
                    loadPics(reed.body());
                    resetRefreshBtn();
                    Log.d("Text","in");
                } catch (IOException e) {
